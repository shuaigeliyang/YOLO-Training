...
def get_training_config_for_many_classes():
    ...
    config = {
        # 基础配置
        'data': './data.yaml',
        'epochs': 5000,  # 增加轮数，因为类别多需要更多学习
        'imgsz': 640,
        'batch': 50,  # 增加批次
        'device': 0,

        # 优化器配置 - 针对多类别调整
        'lr0': 0.0005,  # 更低的学习率，避免震荡
        'lrf': 0.001,  # 最终学习率
        'optimizer': 'AdamW',# 优化器,AdamW通常在复杂任务上表现更好
        'momentum': 0.9,  # 稍低的动量
        'weight_decay': 0.0001,  # 更小的权重衰减

        # 学习率调度
        'cos_lr': True,
        'warmup_epochs': 5.0,  # 更长的热身期
        'warmup_momentum': 0.8,
        'warmup_bias_lr': 0.01,

        # 损失权重 - 针对多类别重调
        'box': 5.0,  # 降低边界框损失权重
        'cls': 2.0,  # 大幅增加分类损失权重（重要！）
        'dfl': 1.0,  # 降低分布焦点损失权重
...
if __name__ == '__main__':
    ...
    try:
        ...
        model = YOLO('models/等级5_5000次.pt')
