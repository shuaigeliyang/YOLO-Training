...
# 检测图片路径
img_path = r"D:\shijue\pythonProject1\xun\YOLO\周5(1)\��5\5-1.JPG"
...
for i, model_path in enumerate(model_paths, 1):
    print(f"正在加载模型 {i}: {model_path}")

    # 加载模型
    model = YOLO(model_path)

    # 为每个模型创建子文件夹
    model_save_dir = os.path.join(save_dir, f"等级{i}")
    ...
    print(f"模型 {i} 检测完成，结果保存在: {model_save_dir}")
...
with open(summary_file, 'w', encoding='utf-8') as f:
    ...
    for detection_group in all_detections:
        model_id = detection_group['model_id']
        detections = detection_group['detections']

        f.write(f"\n模型 {model_id} (等级{model_id}):\n")
        f.write("-" * 30 + "\n")
